...
if (window.localStorage.getItem('_kongTLang_') === null) {
  if (langSub === 'zh') {
    localStorage.setItem('_kongTLang_', 'CN')
    lan = 'CN'
  } else {
    localStorage.setItem('_kongTLang_', 'EN')
    lan = 'EN'
  }
  i18n.locale = lan
} else { // 根据用户上次的选择的语言来显示语言
  i18n.locale = window.localStorage.getItem('_kongTLang_')
}
